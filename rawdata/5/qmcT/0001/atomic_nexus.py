...
scf = generate_pyscf(
    ...
    job          = job(serial=True, nodes=64, queue='overrun', hours=0.5, constraint='knl', presub=scf_presub),
...
c4q = generate_convert4qmc(
    identifier   = 'c4q',
    path         = 'scf',
    job          = job(nodes=64, queue='overrun', hours=1, constraint='knl', presub=scf_presub),
...
optJ12 = generate_qmcpack(
    identifier     = 'optJ12',
    path           = 'optJ12',
    job            = job(nodes=64, queue='overrun', hours=0.5, constraint='knl', presub=scf_presub),
...
gs_vmc = generate_qmcpack(
    identifier   = 'gs_vmc',
    path         = 'gs_vmc',
    job          = job(nodes=NODES, queue='overrun', hours=0.5, constraint='knl', presub=scf_presub),
...
gs_qmc = generate_qmcpack(
    ...
    calculations   = [ 
        ...
        dmc(targetwalkers = 4096,  # Total walkers
            timestep = 0.0005,
            warmupsteps =  int(3.0/0.0005),
            blocks = 20,
            steps = int(2.0/0.0005),
            nonlocalmoves = 'yes',
            checkpoint = 5
            ),
        dmc(targetwalkers = 4096,  # Total walkers
            timestep = 0.0001,
            warmupsteps =  int(3.0/0.0001),
            blocks = 20,
            steps = int(2.0/0.0001),
            nonlocalmoves = 'yes',
            checkpoint = 5
            ),
